...
const OUTPUTREPORT = "reportTest.json";
...
for (let i = 0; i < 10000; i++) {
  ...
  for (let attributeIndex in data.attributes) {
    ...
    if (!groups[trait_type][value]) {
      groups[trait_type][value] = [];
    }
    groups[trait_type][value].push(data.name);
    // console.log(data.name + ": " + attribute.trait_type + " => " + attribute.value);
  }
...
(async () => {
  await fs.writeFile(OUTPUTREPORT, JSON.stringify(groups, null, 2), (err) => {
      if (err) throw err;
